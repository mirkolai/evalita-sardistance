...
class Quote_Network(object):
    G=None
    def __init__(self):

        if os.path.isfile('network/QUOTE.pickle'):
            self.G = pickle.load(open('network/QUOTE.pickle', 'rb'))
        else:
            self.G=nx.Graph()
            ...
            pickle.dump(self.G,open('network/QUOTE.pickle', 'wb'))
